...
s2l_lut = np.float32(tuple(s2l(i/255.0) for i in range(256)))


def rgb2lin(img: np.ndarray) -> np.ndarray:
    img = np.concatenate(
        (cv2.LUT(img[:, :, :3], s2l_lut), img[:, :, 3:4].astype(np.float32)/255.0), axis=2)
    return img


def lin2rgb(img: np.ndarray) -> np.ndarray:
    img = np.clip(img, 0.0, 1.0)
    img[:, :, :3] = np.where(img[:, :, :3] <= 0.0031308, 12.92 *
                             img[:, :, :3], 1.055*img[:, :, :3]**(1.0/2.4) - 0.055)
